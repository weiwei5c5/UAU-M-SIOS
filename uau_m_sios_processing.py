...
if __name__ == '__main__':
    ...
    m_ = 0.5
    # SIOS parameters
    initial_p_, beta_u_, gamma_, mu_1_, mu_2_ = 0.02, 0.3, 0.5, 0.4, 0.4
    # joint parameters
    ...
    for corr in correlation:
        ...
        save_dict[corr] = [mp_i_list_mmca, i_list_mc_mp, mp_o_list_mmca, o_list_mc_mp, mp_a_list_mmca, a_list_mc_mp]
        plt.plot(beta_u_list, mp_i_list_mmca, 'r-', label='ρI_mmca_mp')
        plt.plot(beta_u_list, mp_a_list_mmca, 'y-', label='ρA_mmca_mp')
        plt.plot(beta_u_list, mp_o_list_mmca, 'g-', label='ρO_mmca_mp')
        ...
        plt.plot(beta_u_list, o_list_mc_mp, 'g^', label='ρO_mc_mp')
        plt.xlabel(r'$\beta_U$')
        plt.ylabel(r'$\rho$')
        plt.title("correlation = {0}".format(corr))

        plt.legend()
        # plt.savefig(corr + 'checko_mu2_mp.png')
        plt.savefig(corr + 'mc_mmca_mp.png')
        plt.close()
        # plt.savefig('checko_mu2.png')
    np.save('./data/total_mmca_mc.npy', save_dict)
